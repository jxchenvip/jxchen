...
function copyAllFile() {
    if (!argvs.path) return false;
    var tDir = findPathDir(argvs.path);
    var cwd = process.cwd();
    var arrCwd = cwd.split(path.sep);
    var tempArr = tDir.split(path.sep);
    arrCwd.splice(0, tempArr.length);
    var targetdir = tempArr.concat(arrCwd).join(path.sep);

    var aa = fs.readdirSync(cwd).reduce(function (o, key, i, arr) {
        o[i] = path.join(cwd, key);
        return o;
    }, {});
    ...
    fs.readdirSync(cwd, function (err, list) {
        console.log(list)
    })
...
var jxchen = new Promise(function (resolve, reject) {
    var arg = arguments;
    prompt('项目名称(project name) : ', createProject);
